...
public:
	...
	string AVL::getSearch(string input)
	{
		return input.substr(7, input.length());
	}

	string AVL::getRange(string input)
	{
		string words[5];
	...
	string AVL::getRange2(string input)
	{
		string words[5];
		istringstream ss(input);
		for(int i = 0; i < 5; i++)
	...
	string AVL::getInsert(string input)
	{
		return input.substr(7, input.length());
	}
    ...
    void AVL::insertHelper(string input)
    {
    	insert(input, root);
    }
    node* AVL::insert(string input, node* t)
    {
    	int maxHeight;
        if(t == NULL)
        {
    ...
    node* AVL::search(string query, node* t)
    {
    	if (t == NULL){
	...
	void AVL::rangeSearch(string word1, string word2, node* t) //pass in the root
	{
...
int main(int argc, char* argv[])
{
    ...
    for(int i = result.size(); i > 0; i--)
    {
        if(result[i].substr(0,5) == "search")
            avl.search(avl.getSearch(result[i]));
